...
try {
  ...
  const report = { passed: true, captured_at: new Date().toISOString(), title: result.candidate.name, runtime: result.candidate.runtime,
    checks: ['blank-key-first-run', 'key-field-live-verification', 'key-field-cleared', 'region-and-services', '45-minute-comedy', 'watch-options-http', 'preview-without-saving', 'watchlist-save', 'episode-persistence', 'export-omits-key', 'demo-isolation'] };
  ...
  throw new Error('Live setup journey failed. Check the disposable setup locally; detailed browser errors are withheld to protect the API key.');
